...
- **A result read synchronously via `get_subagent_result` no longer also arrives as a completion notification.** When a background subagent finished while the parent was mid-turn (e.g. blocked in a long tool call), the completion notification was sent after only a 200ms hold — a window the parent physically could not call `get_subagent_result` within — and pi's fire-and-forget `sendMessage` made the queued followUp un-retractable, so the report was delivered twice and the notification's `triggerTurn` burned an extra assistant turn on "that's just the notification, already covered above". Notification sends are now parked while the main session is mid-turn (tracked via `turn_start`/`turn_end`) and released at turn end with the existing 200ms grace, so `resultConsumed`/`cancelNudge` stays effective for the whole turn; an armed nudge whose grace window expires inside a *new* turn re-parks instead of firing. Idle-session behavior is unchanged. Terminal `get_subagent_result` results now also carry the notification details and render as markdown in the TUI (same header/body renderer as the notification), so the synchronous read looks like the notification it suppresses.
