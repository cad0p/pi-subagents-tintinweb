...
- **`checkpoint` tool — subagent-authored progress checkpoints the parent pulls on demand.** A new subagent-facing tool (available to every subagent by default; not added to `EXCLUDED_TOOL_NAMES`) the subagent calls at milestones to save a one-sentence summary of what it just did and what it's doing next. The latest summary is surfaced inline in `get_subagent_result`'s output; the full history lives in a `.checkpoints.md` file appended in `## Turn N/M — Xs elapsed` format. The `.checkpoints.md` file is gated on `record.outputFile` (skipped when `output_transcript: false` suppresses the `.output` transcript, though the in-memory `lastCheckpoint` is still updated), and checkpoints are pull-only — no new model call, no mid-run push to the parent. The tool returns a brief `Checkpoint saved (turn N/M, Xs).` confirmation to the subagent.

### Changed

- **`get_subagent_result` redesigned — surfaces the latest checkpoint and the checkpoint/transcript file paths, and drops the `verbose` parameter.** The parent LLM previously had two ways to inspect a running background subagent: a mid-run `get_subagent_result` that returned `"Agent is still running… check back later"` (zero useful signal), and a post-completion `verbose: true` that dumped the entire conversation transcript (tool results truncated to 200 chars, unstructured firehose). Both are replaced by a single pull-based shape: the running header now carries `turn N/M, Xs elapsed` and the latest checkpoint summary (if any), with both file paths (`.checkpoints.md` and `.output`) for drill-in; the completed header keeps the Status + stats + status-note line and result preview, then appends the latest checkpoint and file paths. The footer discourages polling on the running shape (`Do not poll repeatedly`) and drops it on the completed shape (the agent is done). The `verbose` parameter is removed from the schema — read the `.output` transcript file directly for full detail. The conversation-viewer's truncation hint, which pointed at `verbose`, now points at the transcript file. The `resultConsumed` / `cancelNudge` logic is preserved (it suppresses the completion notification when the parent reads the result).
- **BREAKING: `get_subagent_result` no longer accepts a `verbose` parameter.** The `verbose: true` branch that dumped the agent's full conversation (the `--- Agent Conversation ---` block) is gone; the schema no longer declares it. Migration: orchestrator prompts that passed `verbose: true` should read the `.output` transcript file path that `get_subagent_result` now surfaces instead — it's the full, untruncated transcript.
...
- **Background completion notifications are now markdown reports rendered by pi's default custom-message component.** The fork's `subagent-notification` message renderer is gone; each completion's `content` is the report itself — a bold status line (`**✓ Subagent completed: <description>** · ↻3≤30 · 3 tool uses · 12.4k token · ctx 61.0% of 200k · 4.1s`), `Agent:` / `Transcript:` lines, and the result body under a `Result:` label. pi draws it in its standard extension-message box (violet `customMessageBg` frame + `[subagent-notification]` label) and renders the markdown, so the same text is both the parent model's context and what the user sees. Error and stopped reports carry the failure reason on the status line (`**✗ Subagent error: <description>** — <error>`); descriptions and error text have newlines collapsed so they cannot forge metadata lines; bodies are no longer XML-escaped, so `Promise<T>` reaches the parent intact. `failurePreviewMaxChars` still caps the `Result:` body of `error`/`stopped` runs. **BREAKING:** the `<task-notification>` XML envelope and the structured `details` payload are gone — historical notifications replay as raw text, and session JSONL / RPC `get_messages` / `/export` now surface markdown where they previously returned XML.
- **Background completions are delivered through pi's steering queue, one message per agent.** The fork's join-mode/group-batching machinery is removed: every completion sends its own `{ deliverAs: "steer", triggerTurn: true }` notification and pi's queue owns coalescing. The parked-nudge scheduler (park mid-turn, release at the next `message_end` / `tool_execution_end` boundary) and the `resultConsumed` re-check are unchanged. Coalescing is now a pi setting: `steeringMode: "all"` delivers every completion queued at a boundary in one parent turn; pi's default `"one-at-a-time"` drains one per turn.

### Removed

- **The `subagent-notification` custom message renderer, `NotificationDetails`, and the structured `details` payload on `get_subagent_result` results.** pi's default component renders the markdown report instead. **BREAKING:** consumers that read the structured details (session JSONL, RPC, `--json`) no longer receive them — read the markdown `content` instead.
- **The `<task-notification>` XML envelope** (and its `escapeXml` helper). **BREAKING:** historical notifications in existing sessions replay as raw XML text; no adapter is provided.
- **Join modes and group join** (`smart` / `group` / `async`, the `defaultJoinMode` setting, `GroupJoinManager`, the group completion banner). Every completion is now its own notification; use pi's `steeringMode` to control coalescing. Existing `subagents.json` files keep the dead keys — the sanitizer silently drops them.
- **The `resultPreviewMode` and `resultPreviewExpanded` settings**, and the notification collapse/expand affordance (pi has no native collapse for custom messages). Existing keys are silently dropped on load.

### Fixed

- **A result read synchronously via `get_subagent_result` no longer also arrives as a completion notification — and mid-turn completions are now injected at the next possible moment** ([#13](https://github.com/cad0p/pi-subagents-tintinweb/issues/13)). When a background subagent finished while the parent was mid-turn (e.g. blocked in a long tool call), the completion notification was sent after only a 200ms hold — a window the parent physically could not call `get_subagent_result` within — and pi's fire-and-forget `sendMessage` made the queued followUp un-retractable, so the report was delivered twice and the notification's `triggerTurn` burned an extra assistant turn on "that's just the notification, already covered above". Notification sends are now parked while the main session is mid-turn (tracked via `turn_start`/`turn_end`), so `resultConsumed`/`cancelNudge` stays effective until the send; an armed nudge whose grace window expires inside a *new* turn re-parks instead of firing. Idle-session behavior is unchanged. Terminal `get_subagent_result` results now render their own text as markdown in the TUI, so the synchronous read reads like the notification it suppresses.
  The mid-turn delivery itself is the second half of the fix: parked notifications are released at the next main-session message boundary (`message_end` / `tool_execution_end`) and sent through the steering queue (`deliverAs: "steer"`), which pi's agent loop drains right after such a boundary — before the parent's next LLM call — so a background completion lands right after the parent's current tool call instead of only when the whole turn ends. `turn_end` remains the fallback release point (delivered as a new turn, as before), which also covers the loop's error/abort path where the steering queue is never drained again. The send closures re-check `resultConsumed` at fire time, so a `get_subagent_result(wait: true)` call that resolves mid-turn still suppresses the push; a pull that happens *after* the boundary already fired simply re-reads the same report in-context (same turn, no extra turn).
- **`isolation: "worktree"` agents are now told to stay in the worktree.** The isolation was advisory-only: the agent's cwd was set to the temporary worktree, but append-mode agents inherit the parent's system prompt verbatim — which names the *shared checkout* as the working directory, in detail, before everything else. Observed failure: two parallel worktree agents both `cd`'d to the main checkout for every operation (edits, `git checkout -b`, commit, push), raced on its HEAD — one agent's branch ended up stacked on the other's — and left the main tree checked out on an agent branch, silently redirecting the parent's later `git pull`/`git log`. The system prompt now carries a two-line `<worktree_isolation>` directive (worktree spawns only) naming the copy and the shared parent directory and telling the agent to never edit the latter, even when inherited instructions name it. Plumbed as `RunOptions.worktree` → `PromptExtras.worktree`; rendered by `buildAgentPrompt` in both append and replace modes. Enforcement (blocking bash commands that reference the parent path) is deliberately out of scope — a bash-capable agent can always `cd`, so this fixes the prompt-level root cause, not the theoretical one.
- **The `checkpoint` tool is no longer exposed to the main agent, and works in real subagent sessions again.** The extension factory registered the subagent-facing tool unconditionally, so the root (main) session offered it to the main agent — which could call it but only ever got `Checkpoint failed: agent record not found.` (the main session has no agent record). Registration is now gated to child (subagent-session) activations via the existing first-activation-wins manager-registry discriminator. The same change also fixes checkpoint calls from real subagents: the execute lookup read the in-scope `manager` closure, but every subagent session re-runs the factory with a fresh, empty child manager while records live in the root manager — so the lookup is routed back through the `Symbol.for("pi-subagents:manager")` registry. The checkpoint test suites now drive the production two-activation topology (the root activation owns the records, the child activation registers and calls the tool), pinning the cross-session contract the single-activation harness had masked.
