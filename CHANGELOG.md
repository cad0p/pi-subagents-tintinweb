...
- **`checkpoint` tool — subagent-authored progress checkpoints the parent pulls on demand.** A new subagent-facing tool (available to every subagent by default; not added to `EXCLUDED_TOOL_NAMES`) the subagent calls at milestones to save a one-sentence summary of what it just did and what it's doing next. The summary lands on `record.lastCheckpoint` (the latest only — the full history lives in a `.checkpoints.md` file alongside the `.output` transcript) and is surfaced inline in `get_subagent_result`'s output. The tool finds its own record by matching `ctx.sessionManager.getSessionId()` against `record.sessionId` (set at spawn in `agent-manager.ts`'s `onSessionCreated` callback), so no `agent_id` param is needed and the subagent can't accidentally write to a sibling's record. The `.checkpoints.md` file is appended in `## Turn N/M — Xs elapsed` format (chronological, oldest first) and is gated on `record.outputFile` — when `output_transcript: false` suppresses the `.output` transcript, the `.checkpoints.md` file is skipped too (the in-memory `lastCheckpoint` is still updated for inline display). No new model call, no mid-run push to the parent — checkpoints are pull-only, read via `get_subagent_result`. The tool returns a brief `Checkpoint saved (turn N/M, Xs).` confirmation to the subagent.
