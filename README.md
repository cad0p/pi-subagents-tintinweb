...
Malformed store records are skipped rather than crashing the menu: a job whose shape is invalid (missing id, unknown agent type, unparseable cron) is hidden from the scheduler but kept verbatim in the store file, with a one-time warning naming the file and count. Fix such an entry by editing the file, or delete it to discard the job. Entries that cannot be re-serialized are dropped with a warning, and a non-array `jobs` container or non-object file is rewritten on the next change.
